...
@section('content')
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-12">
                <div class="card mb-4">
                    <form class="card-header pb-0" role="form" action="{{ route('role-update', $roles->id) }}" method="POST"
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" type="text" name="name"
                                                value="{{ old('name', $roles->name) }}" placeholder="Enter roles name">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            @foreach ($allPermissions as $permission)
                                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}"
                                                    {{ in_array($permission->name, $roles->permissions->pluck('name')->toArray()) ? 'checked' : '' }}>
                                                {{ $permission->name }} <br>
                                            @endforeach
                                        </div>
                                    </div>
