...
class IncomeController extends Controller
{
     ...
     */
    private function generateTransactionNumber($account_number_id)
    {
        $date = date('Ymd');

        $lastIncome = Income::where('income_invoice_number', 'like', $account_number_id.'-'.$date.'-%')
            ->orderBy('id', 'desc')
            ->first();
        ...
        if (!$lastIncome) {
            ...
            $lastNumber = (int) substr($lastIncome->transaction_number, -4);
            $number = $lastNumber + 1;
        }

        return $account_number_id.'-'.$date.'-'.str_pad($number, 4, '0', STR_PAD_LEFT);
    }
    ...
    public function create(Request $request, Income $income)
    {
        ...
        $transaction_number = $this->generateTransactionNumber($request->account_number_id);
